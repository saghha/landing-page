...
$project = $wpdb->get_row($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "wp_itaxxion_portfolio_projects` WHERE id=%d AND published=1", $_POST['project_id']));
...
if ($project) {
    ...
    $images = $wpdb->get_results($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "wp_itaxxion_portfolio_images` WHERE project_id=%d AND published=1 ORDER BY ordering ASC", $_POST['project_id']), object);
