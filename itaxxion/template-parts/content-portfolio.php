<?php
$portfolio = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ".$wpdb->prefix."wp_itaxxion_portfolio WHERE id = %d AND published=1", $atts['id'] ) );

if ( isset($portfolio->id) ) {
    $projects = $wpdb->get_results( $wpdb->prepare("SELECT * FROM `". $wpdb->prefix . "wp_itaxxion_portfolio_projects` WHERE portfolio_id=%d AND published=1", $portfolio->id ), object );
    $category = array();
