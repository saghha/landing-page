...
function add_theme_menu_item()
{
    ...
    add_menu_page('Theme page title', 'itaxxion Theme', 'delete_pages', 'theme_settings_general', 'theme_settings_general', 'dashicons-format-image');
...
function itaxxion_activation($oldname, $oldtheme = false)
{
    global $wpdb;
    $sql_wp_itaxxion_portfolio = "
        CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "wp_itaxxion_portfolio` (
    ...
    $sql_wp_itaxxion_portfolio_projects = "
        CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "wp_itaxxion_portfolio_projects` (
            `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
    ...
    $sql_wp_itaxxion_portfolio_images = "
        CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "wp_itaxxion_portfolio_images` (
            `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
    ...
    $wpdb->query($sql_wp_itaxxion_portfolio);
    $portfolios = $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "wp_itaxxion_portfolio`", object);
    if ($wpdb->num_rows <= 0) {
        $ret = $wpdb->insert(
    ...
    $wpdb->query($sql_wp_itaxxion_portfolio_projects);
    $projects = $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "wp_itaxxion_portfolio_projects`", object);
    if ($wpdb->num_rows <= 0) {
        $ret = $wpdb->insert(
            $wpdb->prefix . "wp_itaxxion_portfolio_projects",
            array(
                'id' => 1,
                'portfolio_id' => 1,
                'name' => 'WP itaxxion Themes',
                'description' => 'WP itaxxion Themes is converted from itaxxion Themes HTML, with some fiture included such as slider management for add slide and custom animation, portfolio management, and footer link management.',
                'image_url' => esc_url(get_template_directory_uri()) . '/images/portfolio/wp_itaxxion.jpg',
                'image_path' => '',
    ...
    $wpdb->query($sql_wp_itaxxion_portfolio_images);
    $projects = $wpdb->get_results("SELECT * FROM `" . $wpdb->prefix . "wp_itaxxion_portfolio_images`", object);
    if ($wpdb->num_rows <= 0) {
        $ret = $wpdb->insert(
            $wpdb->prefix . "wp_itaxxion_portfolio_images",
            array(
                'id' => 1,
                'project_id' => 1,
                'name' => 'General Settings',
                'description' => 'To perform general settings in itaxxion template',
        ...
        $ret = $wpdb->insert(
            ...
            array(
                'id' => 19,
                'project_id' => 2,
                'name' => 'itaxxion Template Kecik Framework Skeleton',
                'description' => 'itaxxion template on Skeleton of Kecik Framework',
                'image_url' => esc_url(get_template_directory_uri()) . '/images/portfolio/itaxxion.jpg',
                'image_path' => '',
                'image_type' => 'image/jpeg',
                'published' => 1,
                'ordering' => 5
    ...
    $sliders = get_option('wp_itaxxion_slider');
    if (!is_array($sliders) || count($sliders) <= 0) {
        add_option('wp_itaxxion_slider', array(
            array(
                // Title
    ...
    $facebook = get_option('wp_itaxxion_facebook_url');
    if (empty($facebook)) add_option('wp_itaxxion_facebook_url', 'https://facebook.com/dnaextrim');
    $twitter = get_option('wp_itaxxion_twitter_url');
    if (empty($twitter)) add_option('wp_itaxxion_twitter_url', 'https://twitter.com/dnaextrim');
    $github = get_option('wp_itaxxion_github_url');
    if (empty($github)) add_option('wp_itaxxion_github_url', 'https://github.com/dnaextrim');


    $footer_links = get_option('wp_itaxxion_footer_links');
    if (!is_array($footer_links) || count($footer_links) <= 0) {
        add_option('wp_itaxxion_footer_links', array(
            'MY PORTFOLIO' => array(
                array(
...
add_action("after_switch_theme", "itaxxion_activation", 10, 2);

function itaxxion_deactivation($newname, $newtheme)
{

}

add_action("switch_theme", "itaxxion_deactivation", 10, 2);
